        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: <Widget>[
              ...
              _buildResultsRow('Audio',
                  CardSettingsFilePicker.formatBytes(model.audio.length, 3)),
              _buildResultsRow(
                  'Photo',
                  CardSettingsFilePicker.formatBytes(
                      model.photo?.length ?? 0, 2)),
              _buildResultsRow('Video',
                  CardSettingsFilePicker.formatBytes(model.video.length, 1)),
              _buildResultsRow(
                  'Custom file',
                  CardSettingsFilePicker.formatBytes(
                      model.customFile.length, 0)),
            ],
          ),
        ),
        actions: <Widget>[
